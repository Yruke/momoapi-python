...
class MomoApi(ClientInterface, object):
    ...
    def getAuthToken(self, product, url, subscription_key):
        ...
        response = requests.post(
            ...
            auth=HTTPBasicAuth(
                self.config.userId(product), self.config.APISecret(product)
            ),
            data=data,
            headers=headers,
        )
    ...
    def getTransactionStatus(self, transaction_id, url, subscription_key, **kwargs):
    ...
    @classmethod
    def generateToken(
        cls, host, api_user, api_key, base_url, environment="sandbox", **kwargs
    ):
